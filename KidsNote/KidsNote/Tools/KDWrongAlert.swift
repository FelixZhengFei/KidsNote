...
///错误提示框
class KDWrongAlert: NSObject {
    
    class func show(_ text:String) {
        if text == "" {
            return
        }
        let wrongView = KDWrongAlert.initialization()
        type=HUDType.text
        ...
        KDWrongAlert.hide(animated: true, afterDelay: 1.5)
    }
    ...
    class func show(text:String,detailText:String,toView:UIView,animated:Bool,hideAfterDelay:TimeInterval) {
        ...
        if type != HUDType.text {
            ...
            if hideAfterDelay>0
            {
                KDWrongAlert.hide(animated: animated, afterDelay: hideAfterDelay)
            }
        } else {
            wrongView.label.text=text
            wrongView.detailsLabel.text=detailText
    ...
    class func showSuccess(text:String,detailText:String,toView:UIView,animated:Bool,afterDelay:TimeInterval) {
        KDWrongAlert.showCustomView(text: text, detailText: detailText, image:#imageLiteral(resourceName: "action_success"), toView: toView, animated: animated, afterDelay: afterDelay)
    }
    
    /**显示错误提示*/
    class func showFailure(text:String,detailText:String,toView:UIView,animated:Bool,afterDelay:TimeInterval) {
        KDWrongAlert.showCustomView(text: text, detailText: detailText, image:#imageLiteral(resourceName: "action_fail"), toView: toView, animated: animated, afterDelay: afterDelay)
    }
    ...
    fileprivate class func showCustomView(text:String,detailText:String,image:UIImage,toView:UIView,animated:Bool,afterDelay:TimeInterval) {
        ...
        if type != HUDType.customView {
            ...
            wrongView.show(animated: animated)
            if afterDelay>0 {
                KDWrongAlert.hide(animated: animated, afterDelay: afterDelay)
            }
        }
    }
    ...
    //MARK: 隐藏HUD
    class func hide() {
        KDWrongAlert.hide(animated: false, afterDelay: 0)
    }
