...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        [self.layer addSublayer:[self layerWithLineColor:[UIColor colorWithHex:@"#E6E6E6"]]];
        //添加动画layer
...
- (void)initViewsWithEnsureTitle:(NSString *)title {
    ...
    rerecordingBtn.layer.cornerRadius = 50.0 /2  ;
    rerecordingBtn.layer.borderWidth = 1;
    rerecordingBtn.layer.borderColor = [UIColor colorWithHex:@"#E6E6E6"].CGColor;
    ...
    [reLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(rerecordingBtn.mas_bottom).offset(8 );
...
- (void)initVoiceStateViews {
    ...
    [recordImgView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(self.readyView).offset(10);
        make.size.mas_equalTo(CGSizeMake(50 , 50 ));
    }];
    
    //录音标签
    recordLabel = [[UILabel alloc] initWithFrame:CGRectZero];
    ...
    [recordLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(recordImgView.mas_bottom).offset(5.0 );
    ...
    self.recordTimeLabel.font = [UIFont systemFontOfSize:15.0 ];
    ...
    [self.recordTimeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(self.recordView.mas_top).offset(-3 );
    ...
    [self.pauseBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.replayView).offset(20);
    ...
    [replayBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.finishView).offset(20);
...
- (void)updateState:(PIVoiceRecordViewState)state seconds:(NSUInteger)seconds{
    ...
    switch (self.state) {
        ...
        case PIVoiceRecordViewStateRecording: {
            //录制中
            self.recordTimeLabel.text = [NSString stringWithFormat:@"%ldS",MIN(seconds, 180)];
        }
            break;
...
- (UIView *)recordView {
    if (!_recordView) {
        _recordView = [[RecordView alloc] initWithFrame:CGRectMake(0, 0, 100 , 100 )];
