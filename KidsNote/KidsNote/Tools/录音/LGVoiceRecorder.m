...
- (void)cleanCache {
    NSString *aacRecordFilePath = [NSHomeDirectory()stringByAppendingPathComponent:self.recoedName];
...
+ (NSString *)getDefaultPath:(NSString *)viedoName {
    NSString *aacRecordFilePath = [NSHomeDirectory()stringByAppendingPathComponent:viedoName];
   NSFileManager *tempManager = [NSFileManager defaultManager];

    if ([tempManager fileExistsAtPath:aacRecordFilePath]) {
