...
- (void)hasRcoded {
    NSString * path = [LGVoiceRecorder getDefaultPath:@"felixTest.aac"];
    if (path != nil) {
    NSLog(@"path===%@",path);
    self.localAACUrl = path;
    NSLog(@"%@",[NSString stringWithFormat:@"声音路径：%@", path]);
    [self.voiceRecordView updateState:PIVoiceRecordViewStateFinish seconds:MIN(180, 0)];
    }
}

- (void)initViews {
    //录音视图
    self.voiceRecordView = [[PIVoiceRecordView alloc] initWithEnsureTitle:@"删除" frame:CGRectMake(0,0, kWidth, 100)];
...
- (void)updateRecordTime {
    if (self.recorder.currentTime == 180) {
        [self stopRecordTimer];
        [self.recorder stopRecording];
    }
...
//    }
    ...
    [_recorder cleanCache];
    [self.voiceRecordView updateState:PIVoiceRecordViewStateReady seconds:MIN(180, 0)];
}
...
- (LGVoiceRecorder *)recorder {
    if (!_recorder) {
        _recorder = [[LGVoiceRecorder alloc] initWithName:@"felixTest.aac"];
        ...
        _recorder.audioFinishRecording = ^(NSString *aacUrl, NSUInteger audioTimeLength) {
            ...
            NSLog(@"%@",[NSString stringWithFormat:@"声音路径：%@", aacUrl]);
