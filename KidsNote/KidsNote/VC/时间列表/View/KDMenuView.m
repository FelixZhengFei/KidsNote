...
@interface DKMenuAction()
@property (nonatomic) NSString      *title;
@property (nonatomic) UIImage       *image;
@property (copy, nonatomic)void (^handler)(DKMenuAction *);
@end
@implementation DKMenuAction
+ (instancetype)actionWithTitle:(NSString *)title image:(UIImage *)image handler:(void (^)(DKMenuAction *))handler{
    DKMenuAction *action = [[DKMenuAction alloc] initWithTitle:title image:image handler:handler];
    return action;
}
- (instancetype)initWithTitle:(NSString *)title image:(UIImage *)image handler:(void (^)(DKMenuAction *))handler{
    if (self = [super init]) {
        _title = title;
        _image = image;
...
@interface KDMenuView()<UITableViewDelegate,UITableViewDataSource>
{
    CGPoint          _refPoint;
    UIView          *_refView;
    CGFloat          _menuWidth;
...
@property(nonatomic,copy) NSArray<DKMenuAction *>   *actions;
...
+ (instancetype)menuWithActions:(NSArray<DKMenuAction *> *)actions width:(CGFloat)width atPoint:(CGPoint)point{
    NSAssert(width>0.0f, @"width要大于0");
    KDMenuView *menu = [[KDMenuView alloc] initWithActions:actions width:width atPoint:point];
    return menu;
}
+ (instancetype)menuWithActions:(NSArray<DKMenuAction *> *)actions width:(CGFloat)width relyonView:(id)view{
    NSAssert(width>0.0f, @"width要大于0");
    NSAssert([view isKindOfClass:[UIView class]]||[view isKindOfClass:[UIBarButtonItem class]], @"relyonView必须是UIView或UIBarButtonItem");
    KDMenuView *menu = [[KDMenuView alloc] initWithActions:actions width:width relyonView:view];
    return menu;
}

- (instancetype)initWithActions:(NSArray<DKMenuAction *> *)actions width:(CGFloat)width atPoint:(CGPoint)point{
    if (self = [super init]) {
...
- (instancetype)initWithActions:(NSArray<DKMenuAction *> *)actions width:(CGFloat)width relyonView:(id)view{
    if (self = [super init]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    DKMenuAction *action = _actions[indexPath.row];
