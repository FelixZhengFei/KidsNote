...
/// 基础Mapp
protocol SMBaseMappable: Codable {
}

/// 基础Mapp 宽展
extension SMBaseMappable {
    
    /**字典转模型对象*/
    public static func mapObjectFromDict<T : SMBaseMappable>(_ dict : [String:Any], _ type:T.Type) throws -> T {
    ...
    /**JSONData转模型对象*/
    public static func mapObjectFromJsonData<T : SMBaseMappable>(_ data:AnyObject?, _ type:T.Type) throws -> T {
        if let sData = data {
    ...
    /**JSONString转模型对象*/
    public static func mapObjectFromString<T : SMBaseMappable>(_ JSONString : String, _ type:T.Type) throws -> T {
        guard let jsonData = JSONString.data(using: .utf8) else {
            print(MapError.jsonToDataFail)
            throw MapError.jsonToDataFail
        }
    ...
    /**JSONString转模型对象*/
    public static func mapArrayFromString<T : SMBaseMappable>(_ JSONString : String, _ type:T.Type) throws -> Array<T> {
        guard let jsonData = JSONString.data(using: .utf8) else {
            print(MapError.jsonToDataFail)
            throw MapError.jsonToDataFail
        }
    ...
    /**JSONData转模型数组*/
    public static func mapArrayFromJson<T : SMBaseMappable>(_ data:AnyObject?, _ type:T.Type) throws -> Array<T> {
        if let sData = data {
            guard let jsonData = try? JSONSerialization.data(withJSONObject: sData, options: .prettyPrinted) else {
                print(MapError.jsonToDataFail)
                throw MapError.jsonToDataFail
